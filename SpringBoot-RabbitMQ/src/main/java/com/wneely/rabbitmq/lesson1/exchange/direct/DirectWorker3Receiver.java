...
import static com.wneely.rabbitmq.lesson1.Constant.DIRECT2_QUEUENAME;

@Slf4j
@Component("directWorker3Receiver")
public class DirectWorker3Receiver {

    @RabbitListener(queues = DIRECT2_QUEUENAME)
    public void receiverMessage(Object message, Channel channel,@Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) throws IOException {
        log.info("接受到消息，消息队列名称为{}，消息为{}，deliveryTag为{}", DIRECT2_QUEUENAME, message, deliveryTag);
        //第一个参数表示消息的唯一标识，
        // 第二个参数控制确认范围：
